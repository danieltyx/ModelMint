...
class WalletConnectProvider extends base_provider_namespaceObject.BaseProvider {
  ...
  async lookupNetwork(connector) {
    ...
    const connectedHexChainId = (0,base_namespaceObject.isHexStrict)(connector.chainId.toString()) ? connector.chainId : `0x${connector.chainId.toString(16)}`;
  ...
  async onConnectorStateUpdate(connector) {
    connector.on("session_update", async (error, payload) => {
      ...
      if (connectedChainId && this.state.chainId !== connectedHexChainId) {
        const maybeConfig = (0,base_namespaceObject.getChainConfig)(base_namespaceObject.CHAIN_NAMESPACES.EIP155, connectedChainId) || {};
...
async function sendJrpcRequest(signClient, chainId, method, params) {
  ...
  if (!session) {
    throw external_eth_rpc_errors_namespaceObject.ethErrors.provider.disconnected();
  }
...
function walletConnectV2Utils_getProviderHandlers(_ref) {
  let {
    connector,
    chainId
  } = _ref;
...
function WalletConnectV2Provider_ownKeys(object, enumerableOnly) { var keys = Object.keys(object); if (Object.getOwnPropertySymbols) { var symbols = Object.getOwnPropertySymbols(object); enumerableOnly && (symbols = symbols.filter(function (sym) { return Object.getOwnPropertyDescriptor(object, sym).enumerable; })), keys.push.apply(keys, symbols); } return keys; }
function WalletConnectV2Provider_objectSpread(target) { for (var i = 1; i < arguments.length; i++) { var source = null != arguments[i] ? arguments[i] : {}; i % 2 ? WalletConnectV2Provider_ownKeys(Object(source), !0).forEach(function (key) { defineProperty_default()(target, key, source[key]); }) : Object.getOwnPropertyDescriptors ? Object.defineProperties(target, Object.getOwnPropertyDescriptors(source)) : WalletConnectV2Provider_ownKeys(Object(source)).forEach(function (key) { Object.defineProperty(target, key, Object.getOwnPropertyDescriptor(source, key)); }); } return target; }
...
class WalletConnectV2Provider extends base_provider_namespaceObject.BaseProvider {
  constructor(_ref) {
    ...
    super({
      config: {
        chainConfig: WalletConnectV2Provider_objectSpread(WalletConnectV2Provider_objectSpread({}, config.chainConfig), {}, {
          chainNamespace: base_namespaceObject.CHAIN_NAMESPACES.EIP155
        }),
        skipLookupNetwork: !!config.skipLookupNetwork
      },
      state: WalletConnectV2Provider_objectSpread(WalletConnectV2Provider_objectSpread({}, state || {}), {}, {
        chainId: "loading",
        accounts: []
      })
    });
  ...
  async setupEngine(connector) {
    ...
    const providerHandlers = walletConnectV2Utils_getProviderHandlers({
      connector,
      chainId: numChainId
    });
    ...
    connector.events.on("session_event", async payload => {
      ...
      if (event.name === "chainChanged") {
        ...
        if (connectedHexChainId && this.state.chainId !== connectedHexChainId) {
          ...
          // Handle rpcUrl update
          this.configure({
            chainConfig: WalletConnectV2Provider_objectSpread(WalletConnectV2Provider_objectSpread({}, maybeConfig), {}, {
              chainId: connectedHexChainId,
              rpcTarget: rpcUrl,
              chainNamespace: base_namespaceObject.CHAIN_NAMESPACES.EIP155
            })
  ...
  try {
    ...
    switch (parameters.version) {
      ...
      case eth_sig_util_namespaceObject.SignTypedDataVersion.V4:
        ...
        external_assert_default().ok(data.primaryType in data.types, `Primary type of "${data.primaryType}" has no type definition.`);
        const validation = external_jsonschema_default().validate(data, eth_sig_util_namespaceObject.TYPED_MESSAGE_SCHEMA.properties);
...
function ethPrivatekeyUtils_getProviderHandlers(_ref) {
  ...
  return {
    getAccounts: async _ => [`0x${(0,util_namespaceObject.privateToAddress)(Buffer.from(privKey, "hex")).toString("hex")}`],
    ...
    processTypedMessage: async (msgParams, _) => {
      ...
      const params = ethPrivatekeyUtils_objectSpread(ethPrivatekeyUtils_objectSpread({}, msgParams), {}, {
...
class TransactionFormatter {
  ...
  async estimateTxGas(txMeta) {
    const txParams = TransactionFormatter_objectSpread({}, txMeta);
...
class EthereumPrivateKeyProvider extends base_provider_namespaceObject.BaseProvider {
  ...
  async lookupNetwork() {
    ...
    if (parseInt(chainId, 16) !== parseInt(network, 10)) throw external_eth_rpc_errors_namespaceObject.ethErrors.provider.chainDisconnected(`Invalid network, net_version is: ${network}`);
